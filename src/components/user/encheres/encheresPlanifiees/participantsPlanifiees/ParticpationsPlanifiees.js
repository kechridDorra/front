import "./ParticipantsPlanifiees.css";
...
const ParticipantsPlanifiees = () => {
  const userInfo = localStorage.getItem("user-info");
  const [list, setList] = useState({});
  const { enchere } = useParams();
  const navigate = useNavigate();
  async function ParticipantsPlanifiees() {
    try {
...
export default ParticipantsPlanifiees;
