...
function Body() {
  const [encheresT, setEncheresT] = useState();
  const pathImg = "http://localhost/pfe_backend/public/uploads/";

  async function getEncheresT() {
    try {
      const res = await Api.get(`/encheresT`);
      // const users =await   axios.get(`https://127.0.0.1:8000/users`)
      setEncheresT(res.data);
    } catch (error) {
      console.log(error);
    }
  }
  ...
  return (
    <>
      ...
      <section class="container py-2">
        ...
        <div class="row">
          <div class="col-12 col-md-3 p-5 mt-1">
            <a href="#">
              <img
      ...
      <section class="bg-light">
        <div class="container py-3" enctype="multipart/form-data">
          <div class="row text-center pt-0">
            <div class="col-lg-6 m-auto">
              <h1 class="h1">Les enchères terminées</h1>
            </div>
          </div>
<br></br>
          <div class="col-lg-4 m-auto"></div>
          {encheresT && encheresT.length > 0 ? (
            <div class={"row"}>
              {encheresT.map((enchere) => (
                <div class="col-8 col-md-4 mb-3" key={enchere.id}>
                  <div class="card h-70">
                    <a href="/login">
