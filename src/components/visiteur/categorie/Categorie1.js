...
class Categorie1 extends Component {
  constructor() {
    super();
    this.state = { categorie1: [], loading: true };
  }
  componentDidMount() {
    this.getCategorie1();
  }
  async getCategorie1() {
    try {
      const categorie1 = await Api.get(`/categorie1`);
      this.setState({ categorie1: categorie1.data, loading: false });
      console.log("categorie", categorie1);
    } catch (error) {
      // alert("test")
    }
  }
  render() {
    const { loading, categorie1 } = this.state;
          ...
          <div class="container py-5" enctype="multipart/form-data">
            <nav aria-label="breadcrumb">
              <ol class="breadcrumb bg-transparent pl-0 mb-0">
                ...
                <li class="breadcrumb-item">
                  <a href="/categorie1">
                    Catégorie : <strong> {categorie1.nom}</strong>
                  </a>
                </li>
              </ol>
            </nav>
            ...
            {loading ? (
              ...
              <div class={"row"}>
                {this.state.categorie1.encheres.map(encheres =>
